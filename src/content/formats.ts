...
export const credentialFormats: CredentialFormat[] = [
  ...
  },
  {
    slug: 'sub2api',
    title: 'Sub2API',
    shortDescription: 'AI API gateway account/configuration data modeled by AuthAtlas for credential inspection.',
    definition: 'Sub2API is an AI API gateway platform for distributing and managing AI subscription quota. In AuthAtlas, a “Sub2API credential configuration” means the version-dependent account or configuration data that carries provider metadata and upstream authentication material for that gateway; it is an AuthAtlas modeling term, not a separate secret type defined by the upstream project.',
    ...
    security: ['Treat gateway configuration as secret whenever it contains upstream credentials.', 'Verify version-specific schema before import.', 'Do not send live gateway configuration to untrusted conversion services.'],
    faq: [
      { question: 'Is Sub2API itself a new credential type?', answer: 'No. The upstream project is an AI API gateway platform. AuthAtlas uses “Sub2API credential configuration” as a model for gateway account/configuration data that may contain upstream OAuth or API-key material.' },
      { question: 'Can Sub2API be converted to New API?', answer: 'Potentially through schema mapping when both deployed versions can represent the required upstream credential material. This does not create a new secret.' },
    ],
    references: [
  ...
  },
  {
    slug: 'new-api',
    title: 'New API',
    shortDescription: 'AI API gateway channel configuration modeled by AuthAtlas for credential inspection.',
    definition: 'New API is an AI API gateway and usage-management system. Its upstream channels are configuration units for connecting AI providers. In AuthAtlas, a “New API credential configuration” means the provider/channel configuration data that contains or references upstream authentication material; exact fields vary by provider, channel type, and deployed project version.',
    sensitivity: 'Depends on contents',
    lifetime: 'Mirrors upstream credential',
    ...
    conversions: 'Compatible credential material can be extracted into Canonical format or schema-mapped to another gateway format after checking the exact source and target versions.',
    security: ['Channel configuration may contain live provider secrets.', 'Validate mappings against the deployed New API version and channel type.', 'Mask secrets in screenshots, logs, and issue reports.'],
    faq: [
      { question: 'Does every New API channel use the same credential fields?', answer: 'No. Official New API documentation describes channels as provider-connection configuration units, and their settings vary with provider and channel behavior. AuthAtlas therefore treats mappings as versioned adapters.' },
      { question: 'Can New API be converted to Sub2API?', answer: 'Potentially through schema mapping when the required upstream credential material exists in the source and the verified Sub2API target schema can represent it.' },
    ],
    references: [
