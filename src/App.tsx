...
function parseInput(raw: string): Detection {
  ...
  try {
    ...
    if (accessToken && refreshToken) return { label: 'OpenAI OAuth Credential', authType: 'OAuth 2.0', provider, confidence: 98, risk: 'Highly sensitive', fields, accessToken, refreshToken }
...
function ConverterPage({ input, setInput, detection }: { input: string; setInput: (value: string) => void; detection: Detection }) {
  ...
  const copyOutput = async () => { await navigator.clipboard.writeText(output); setCopied(true); window.setTimeout(() => setCopied(false), 1200) }
  ...
  return <main className="page-shell inner-page converter-page"><section className="content-width page-heading converter-heading"><div><span className="breadcrumb">Home <ArrowRight /> Converter</span><h1>Credential <span>Converter</span></h1><p>Convert OAuth tokens, API keys, and credentials between formats locally — your data never leaves your device.</p></div><LocalBadge /></section><section className="content-width conversion-strip"><div className="format-select"><small>Source format</small><strong><RefreshCw size={16} /> Auto-detect <ChevronDown size={14} /></strong></div><div className="detected-chip"><CheckCircle2 size={16} /> Detected: {detection.refreshToken ? 'Refresh Token' : detection.label}</div><ArrowRight className="strip-arrow" /><div className="format-select"><small>Target format</small><strong><Box size={16} /> {target} <ChevronDown size={14} /></strong><select aria-label="Target format" value={target} onChange={(event) => setTarget(event.target.value as Target)}><option>Sub2API</option><option>New API</option><option>Canonical</option></select></div><ArrowRight className="strip-arrow" /><button className="button primary convert-button" type="button"><Zap size={17} /> Convert</button></section><section className="content-width converter-workspace"><article className="workspace-card input-card"><div className="workspace-head"><strong><span>1</span> Input</strong><div><button type="button" onClick={() => setShowSecret((v) => !v)}>{showSecret ? <EyeOff /> : <Eye />} {showSecret ? 'Mask' : 'Reveal'}</button><button type="button" onClick={() => setInput('')}><Trash2 /> Clear</button></div></div><textarea value={input} onChange={(event) => setInput(event.target.value)} spellCheck={false} aria-label="Credential source" className={showSecret ? '' : 'mask-soft'} /><div className="workspace-status"><span><CheckCircle2 /> JSON detected</span><span>1.24 KB · Format detected</span></div></article><article className="workspace-card output-card"><div className="workspace-head"><strong><span>2</span> Output ({target})</strong><button type="button" onClick={copyOutput}><Copy /> {copied ? 'Copied' : 'Copy'}</button></div><CodeBlock value={visibleOutput} /><div className="workspace-status"><span><CheckCircle2 /> Valid {target}</span><span>JSON</span></div></article><aside className="converter-aside"><div className="side-card compatibility-note"><div className="side-title"><strong><Info /> Compatibility note</strong><span className="chip purple">Schema map</span></div><p>This conversion maps fields into the target schema. No tokens are exchanged.</p><dl><div><dt>Strategy</dt><dd>Schema map</dd></div><div><dt>Confidence</dt><dd><span className="green-text">High</span></dd></div><div><dt>Fidelity</dt><dd><span className="green-text">High</span></dd></div><div><dt>Data leaves device</dt><dd><span className="green-text">Never</span></dd></div></dl></div><div className="side-card warning-card"><div className="side-title"><strong><CircleAlert /> Warnings</strong><span className="chip amber">2</span></div><div className="warning-item"><CircleAlert /><div><strong>Missing optional fields</strong><p>Some gateway versions may require additional metadata.</p></div></div><div className="warning-item"><CircleAlert /><div><strong>Version-dependent schema</strong><p>Verify field names against the deployment version.</p></div></div></div></aside></section><section className="content-width converter-support-grid"><article className="support-card"><div className="support-title"><Eye /> Masked preview <span className="chip purple">Masked</span></div><SecretRow label="access_token" value={detection.accessToken} /><SecretRow label="refresh_token" value={detection.refreshToken} /><p className="support-foot"><CheckCircle2 /> Sensitive values are masked locally.</p></article><article className="support-card"><div className="support-title"><ShieldCheck /> Validation</div><ValidationRow label="JSON syntax" value="Valid" /><ValidationRow label="Required fields" value="Present" /><ValidationRow label="Schema" value={`${target} target`} /><ValidationRow label="Processing" value="Browser only" /></article><article className="support-card export-card"><div className="support-title"><Box /> Export</div><p>Save your converted credential as a local file or copy it to the clipboard.</p><div className="export-actions"><button className="button primary" onClick={downloadOutput}><Download /> Download JSON</button><button className="button secondary" onClick={copyOutput}><Copy /> Copy to clipboard</button></div></article></section></main>
}

function SecretRow({ label, value }: { label: string; value?: string }) { return <div className="secret-row"><span><small>{label}</small><code>{mask(value)}</code></span><Copy size={16} /></div> }
